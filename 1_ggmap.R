...
packages_1=c("XML","ggmap","ggplot2","igraph","quantmod","scales");
#packages that are required to install
...
raw <- tables[[6]];  
#查看表格的第一行数据，如果网页编码不规范（例如使用GB2312）会出现乱码
raw[1,];
# 只保留时间、经度、纬度这三列数据，并存入变量data。
data <- raw[ ,c(1,3,4)];  
# 修改data包含的表格各列的名称为'date'、'lan'和'lon'。
names(data) <- c('date','lan','lon');
# 将经度(data$lan)和纬度(data$lon)的数据类型用函数as.numeric转换为数值类型
...
p1 <- p1 + geom_point( aes( plot_X, plot_Y, colour = log10_p_value, size = plot_size), alpha = I(0.6) );   #确定X轴、Y轴、颜色、大小和透明度的映射规则
p1 <- p1 + scale_colour_gradientn (colours = c("blue", "green", "yellow", "red"), limits = c( min(one.data$log10_p_value), 0) );  #确定颜色过度的标度控制
p1 <- p1 + geom_point( aes(plot_X, plot_Y, size = plot_size), shape = 21, fill = "transparent", colour = I (alpha ("black", 0.6) ));      #添加点图对象，并设置颜色
...
p1 <- p1 + theme(legend.key = element_blank());   #添加图例说明
#以下是设置X轴和Y轴的刻度限
